...
public class WeatherTCP implements Weathers{
	
	private static final Log log = Logs.getLog(Modbus.class); 
	
	private static final byte[] cmd = {0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x0C, 0x03, (byte)0x9C, (byte)0xAE, 0x00, 0x0B};
	
	private WeatherInfo w;
	
	public WeatherTCP(){}
	
	public WeatherTCP(WeatherInfo w){
		this.w = w;
	}
	
	private static final int WTIME = PropsUtil.getInteger("weather.wait.time", 5000);
	
	private static final int STIME = PropsUtil.getInteger("weather.sleep.time", 10000);
