 ...
 * @date	2017-10-01
 */
public class SmartOnew implements SmartModel{
	
	//private static final byte[] WRITES = new byte[]{0x00, 0x00, 0x00, 0x00, 0x00, 0x1F, 0x01, 0x10, (byte)0xD9, 0x3A, 0x00, 0x0C, 0x18, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00, 0x12, 0x00};
	//写单个指令
	...
	//			cmd[reg] = CtrConstant.M2O;
				...
				for (int i = 0; i < 6; i++) {
					...
					if(req != null && req.length == 12 && CtrConstant.M2O == req[11]){
						//前台同步讯息
						DwrUtil.sendCtr(JsonUtil.toJson(new CtrMsg(equip.getHouseNo(), equip.getEquipNo(), CtrConstant.C_ING, equip.getModel(), CtrConstant.R2O, equip.getEquipName() + "：正在开启，请稍候！")));
						//同步当前设备状态
...
//			cmd[reg] = CtrConstant.M2S;
			...
			for (int i = 0; i < 6; i++) {
				...
				if(req != null && req.length == 12 && CtrConstant.M2S == req[11]){
					//前台同步讯息
					DwrUtil.sendCtr(JsonUtil.toJson(new CtrMsg(equip.getHouseNo(), equip.getEquipNo(), CtrConstant.C_ING, equip.getModel(), CtrConstant.M2S, equip.getEquipName() + "：正在关闭，请稍候！")));
					//同步当前设备状态
			...
			for (int i = 0; i < 30; i++) {
				...
				if(req != null){
					if(req[index] == CtrConstant.R2SA){
						//前台同步讯息
						DwrUtil.sendCtr(JsonUtil.toJson(new CtrMsg(equip.getHouseNo(), equip.getEquipNo(), CtrConstant.C_CLOSE, equip.getModel(), CtrConstant.R2SA, equip.getEquipName() + ":风机已停止！")));
						//同步当前设备状态
				...
				if(i == 29){
					throw new RuntimeException("关风机状态查询超出步长");
				}
			}
			if(Strings.isNotBlank(equip.getBindIp())){
				//关闭绑定风窗
