 ...
 * @date	2017-07-18
 */
public class TempOne extends Thread{
	...
	private static final Log log = Logs.getLog(TempOne.class);
	...
	public TempOne(String houseNo, int reportFlag){
		try {
			this.houseNo = houseNo;
			datas = new TestData(TypeCheck.ONE.val(), Times.now(), houseNo, null);
			...
			datas.setReportFlag(reportFlag);
			...
			log.info("初始化"+ houseNo + "仓房检测信息失败");
			ChecksUtil.ONES.remove(houseNo);
		}
		ChecksUtil.WAITS.add(houseNo);
		DwrUtil.sendOne(TempUtil.getOneJson());
		
	}
	
	public void check(){
		try {
			ChecksUtil.WAITS.remove(houseNo);
			ChecksUtil.ONES.add(houseNo);
			...
			if(points == null || points.size() == 0){
				...
				//验证该仓房是否正在检测
				if(ChecksUtil.ONEINGS.contains(houseNo)){
					addError("仓房正在执行手动检测!");
					insertAll();
				} else {
					ChecksUtil.ONEINGS.add(houseNo);
					DwrUtil.sendOne(TempUtil.getOneJson());
					//获取外温外湿
					getOutTH();
					//获取内温内湿
			...
			ChecksUtil.ONEINGS.remove(houseNo);
			try {
	...
	private void getTemps(){
		...
		for (int i = 0; i < 4; i++) {
			...
			if (Strings.isNotBlank(tb.getIp())) {
				tag = true;
				getTemp(allTemps, i, tb.getBoardType(), tb.getIp(), tb.getPort());
			}
		}
		if(!tag){
			addError("未配置测温板信息!");
	...
	private void insertAll(){
		try{
			...
			if(an0 != null && an0.getNums() > 0){
				...
				MsgUtil.insertMsg(houseNo, "温度高限报警！报警点数：" + an0.getNums(), TypeMsg.WARN.val(), Constant.W_TEMP, datas.getId());
			}
			if(an1 != null && an1.getNums() > 0){
				...
				MsgUtil.insertMsg(houseNo, "温度升高率报警！报警点数：" + an1.getNums(), TypeMsg.WARN.val(), Constant.W_TEMP, datas.getId());
			}
			if(an2 != null && an2.getNums() > 0){
				...
				MsgUtil.insertMsg(houseNo, "温度异常点报警！报警点数：" + an2.getNums(), TypeMsg.WARN.val(), Constant.W_TEMP, datas.getId());
			}
			if(an3 != null && an3.getNums() > 0){
				...
				MsgUtil.insertMsg(houseNo, "层均温报警！报警层数：" + an3.getNums(), TypeMsg.WARN.val(), Constant.W_TEMP, datas.getId());
			}
			if(an4 != null && an4.getNums() > 0){
				...
				MsgUtil.insertMsg(houseNo, "缺点率报警！缺点率：" + (an4.getNums()/10.0D) + "%", TypeMsg.WARN.val(), Constant.W_TEMP, datas.getId());
			}
			if(an5 != null && an5.getNums() > 0){
				...
				MsgUtil.insertMsg(houseNo, "冷芯报警！报警点数：" + an5.getNums(), TypeMsg.WARN.val(), Constant.W_TEMP, datas.getId());
			}
			//插入异常集合
			dao.insert(ls);
			//插入温度数据
	...
	private void doAllwarns(){
		...
		//缺点率
		if(att.isWarns(Constant.IDX_T_4)){//缺点数=配置总点数-无效点数-有效点数
