 ...
 * @date	2017-07-18
 */
public class TempTime extends Thread{
	...
	private static final Log log = Logs.getLog(TempTime.class);
	...
	public TempTime(String houseNo, Date testDate, String planCode, int reportFlag){
		...
		datas = new TestData(TypeCheck.TIME.val(), testDate, houseNo, planCode);
	...
	private void check(){
		...
		if(points == null || points.size() == 0){
			...
			//验证该仓房是否正在检测
			if(ChecksUtil.TIMEINGS.contains(houseNo)){
				addError("仓房正在执行定时检测!");
				insertAll();
			} else {
				ChecksUtil.TIMEINGS.add(houseNo);
				//获取外温外湿
				getOutTH();
				//获取内温内湿
				getInTH();
		...
		ChecksUtil.TIMEINGS.remove(houseNo);
	...
	private void doAllwarns(){
		...
		//冷芯
		if(att.isWarns(Constant.IDX_T_5) && (ccT/ccN)/10.0D > att.getThd(Constant.IDX_T_5)){
			an5.setNums(ccT/ccN);
