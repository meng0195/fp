...
@Comment("计划任务表")
@Table("PlanTask")
public class PlanTask extends IdEntry {
	...
	public void initCronAndDesc(){
		...
		Date day = DateUtil.parse("yyyy-MM-dd", timeTwo);
		
		if(timeOne == TypeTime1.H.val()){//每小时
			sb1.append("0 0 */1 * * ?");
		}else{
			...
			}else if(timeOne == TypeTime1.C.val()){//
				sb1.append(getDay(day))
				.append(" ")
				.append(getMon(day)+1)
				.append(" ? ")
				.append(getYear(day));
				sb.append(timeTwo).append(" ").append(timeThree);
			}
		}
