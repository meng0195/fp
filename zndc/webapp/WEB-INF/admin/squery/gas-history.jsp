...
<form id="pagerForm" onsubmit="return navTabSearch(this);" action="${CTX_PATH}/squery/gas/history" method="post">
	...
	<div class="searchBar">
		<table class="searchContent">
			<tr>
				<th>仓房：</th>
				<td>
					<select id="squery-gas-history-houseNo" name="ft_EQ_S_houseNo" class="combox">
				...
				<td>
			  	...
			  	<th>检测时间：</th>
				<td>
					<input class="date" dateFmt="yyyy-MM-dd" name="gasDate" value="${gasDate}" readonly="readonly">
			  	</td>
...
<table class="table" layoutH="94">
	<thead>
		<tr>
			...
			<th>开始时间</th>
			<th>结束时间</th>
			<th width="100">CO2平均值(ppm)</th>
			<th width="100">PH3平均值(ppm)</th>
			<th width="100">O2平均值(%)</th>
	...
	<tbody>
		<c:forEach var="u" items="${page.result}" varStatus="st">
			<tr target="uid" rel="${u.id}">
				...
				<td class="right">${u.avgCO2}</td>
				<td class="right">${u.avgPH3}</td>
				<td class="right">${u.avgO2}</td>
				<td class="center">${map:get(enums['TYPE_FLAG'], u.testTag)}</td>
				<td class="center">
					<c:choose>
						<c:when test="${u.curveFlag == 1}">
							<a target="ajaxTodo" title="是否取消标记为曲线数据，请确认?" href="${CTX_PATH}/squery/gas/curve/stop/${u.id}?tid=${param.tid}" class="font-red" >已标记</a>
						</c:when>
						<c:otherwise>
							<a target="ajaxTodo" title="是否标记为曲线数据，请确认?" href="${CTX_PATH}/squery/gas/curve/start/${u.id}?tid=${param.tid}" class="font-blue" >未标记</a>
						</c:otherwise>
					</c:choose>
					<a href="" title="">&nbsp;</a>
				</td>
				<td class="center">
					<a target="dialog" rel="dialog-house-detail" width="1050" height="550" mask="true" href="${CTX_PATH}/squery/show/0/${u.id}/4" title="检测详情" class="font-blue">查看</a>
