...
<form id="pagerForm" onsubmit="return navTabSearch(this);" action="${CTX_PATH}/report/prints" method="post">
	...
	<div class="searchBar">
		<table class="searchContent">
			<tr>
			  	<td>检测日期：</td>
				<td>
					<input id="print-all-date" class="date" dateFmt="yyyy-MM-dd" name="testDate" value="${testDate}" readonly="readonly">
			  	</td>
...
<div class="panelBar">
	<ul class="toolBar">
		<li><a class="print" onclick="prints()" ><span>批量打印</span></a></li>
	</ul>
</div>
<style>
tr.report th{background: #f0f0f0;border: solid 1px #c2c2c2;text-align: center}
</style>
<table class="table" width="1780px" layoutH="65">
	<tr class="report">
...
<div style="position: relative;" id="printAllDiv"></div>
</div>
</div>
<script type="text/javascript">
function prints(){
	if(!$("#print-all-date").val()) {
		alertMsg.error("请选择一天作为打印参数！");
		return;
	}
	$.ajax({
		type : 'POST',
		url : '${CTX_PATH}/report/print/all',
		data : {testDate:$("#print-all-date").val()},
		dataType : "html",
		cache : false,
		success : function(html){
			$("#printAllDiv").html(html);
			if(html.indexOf("<div>请核对输入的信息</div>") < 0){
				setTimeout(function(){
					$.printBox('printAllDiv');
				},700);
			}
		},
		error : DWZ.ajaxError
