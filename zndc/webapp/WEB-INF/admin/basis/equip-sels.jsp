...
<div class="pageHeader" style="padding: 2px;">
	<div class="subBar">
		<a class="font-blue" multlookup="q" warn="请选择设备" style="cursor: pointer;">确认</a>
	</div>
</div>
<table class="edit">
	<c:forEach var="equip" items="${list}" varStatus="st">
		<c:if test="${st.index % 4 == 0}"><tr></c:if>
		<td><label><input type="radio" name="q" value="{${type == 0 ? 'openEquipNo' : 'closeEquipNo'}:'${equip.equipNo}', ${type == 0 ? 'openEquipName' : 'closeEquipName'}:'${equip.equipName}'}" ${equip.equipNo == equipNo ? 'checked' : ''}/>${equip.equipName}</label></td>
